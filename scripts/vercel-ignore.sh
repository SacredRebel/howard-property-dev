...
if git diff --quiet HEAD^ HEAD -- . ':(exclude)data/research.json' ':(exclude)data/uploads.json' ':(exclude)data/zone-positions.json' ':(exclude)images/uploads' ':(exclude)docs/uploads'; then
