...
export function readFrom(rec) {
  ...
  dims.push({ id: 'value', label: 'Value signal', value: total ? total + ' assessed' : '—', score: null, note: [land ? 'land ' + land : '', per ? per + '/ac' : '', /^\$/.test(sale) ? 'last sale ' + sale.split(' — ')[0] : '', dd ? 'last document ' + dd : '', get('value_change') ? '2018 → now ' + (get('value_change').match(/\(([^)]+)\)/) || [])[1] : ''].filter(Boolean).join(' · '), keys: ['total_value', 'land_value', 'value_per_ac', 'doc_nr', 'sale_price', 'value_change'] });
  // 7 change over time
