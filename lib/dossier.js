...
export async function resolveCore(q) {
  ...
  if (rec.apn10 && county) { sideNames.push('provider'); side.push(within(providerTitle({ apn: rec.apnPretty, apn10: rec.apn10, fips: county.fips, state: county.state, lat: site.cy, lon: site.cx, situs: rec.situs }), 7000).then((v) => { provider = v; })); }
  let records = [];
