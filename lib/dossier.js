...
export const COUNTY_ADAPTERS = {
  '06111': {
    ...
    apnPattern: /^\d{3}-?\d-?\d{3}-?\d{3}$/,          // 037-0-012-125
  ...
  '06037': {
    ...
    apnPattern: /^\d{4}-?\d{3}-?\d{3}$/,               // 2048-011-048
    normalizeApn: (s) => String(s).replace(/[^0-9]/g, ''),
...
export function adapterForApn(apn) {
  const s = String(apn || '').trim();
  return Object.values(COUNTY_ADAPTERS).find((a) => a.apnPattern.test(s)) || null;
}
...
async function anchor(q) {
  ...
  if (q.apn) {
    adapter = (q.county && COUNTY_ADAPTERS[q.county]) || adapterForApn(q.apn);
    if (!adapter) { const e = new Error('No county adapter recognises this APN format — pass county=<FIPS> or a point'); e.status = 400; throw e; }
    const key = adapter.normalizeApn(q.apn);
    const rows = await agsQuery(adapter.parcels.root, adapter.parcels.svc, adapter.parcels.layer, null, { where: adapter.parcels.apnField + "='" + key + "'", returnGeometry: true });
    parcel = rows[0] || null;
    if (!parcel) { const e = new Error('APN ' + q.apn + ' is not in the ' + adapter.name + ' parcel layer'); e.status = 404; throw e; }
