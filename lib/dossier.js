...
export async function resolveCore(q) {
  ...
  let evidence = null, provider = null, watch = null;
  if (rec.apn10) { names.push('evidence'); secs.push('title'); tasks.push(async () => { evidence = await loadEvidence(rec.apn10); return []; }); }
  if (rec.apn10) { names.push('watch'); secs.push('title'); tasks.push(async () => { watch = await loadWatch(rec.apn10); return []; }); }
  if (rec.apn10 && county) { names.push('provider'); secs.push('title'); tasks.push(async () => { provider = await providerTitle({ apn: rec.apnPretty, apn10: rec.apn10, fips: county.fips, state: county.state, lat: site.cy, lon: site.cx, situs: rec.situs }); return []; }); }
  let records = [];
  ...
  const diag = q.debug ? done.map((r, i) => [names[i], r.ok ? 'ok' : (r.late ? 'late' : 'error: ' + r.error), r.ms || null])
    .concat(sideNames.map((nm) => [nm + ' (beside the fan-out)', nm === 'evidence' ? (evidence ? 'ok' : 'none on file') : nm === 'watch' ? (watch ? 'ok' : 'not watched') : (provider ? 'ok' : 'no key or no match'), null])) : undefined;
